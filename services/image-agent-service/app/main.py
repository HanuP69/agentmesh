"""Image Agent Service — handles image modality retrieval + CLIP."""
import logging
import os
...
from shared.embeddings import embed_image_text, configure_clip, configure_embedder
from shared.fusion import AgentResult

logger = logging.getLogger(__name__)
app = FastAPI(title="AgentMesh Image Agent Service")
...
for i in range(settings.SHARD_NODES):
    rings.add_node("image", f"shard-{i}")
...
reranker = Reranker(llm_client=_SynthesizerProxy())

class ImageAgent:
    modality = "image"
    def __init__(self, rings, store, hybrid, reranker):
        self.rings = rings; self.store = store; self.hybrid = hybrid; self.reranker = reranker
    def _shards(self):
        return self.rings.rings[self.modality].nodes
    def retrieve(self, query, top_k=5, use_llm_rerank=False, embed_query_text=None):
        try:
            qvec = embed_image_text(embed_query_text or query)
            has_dense = True
        except Exception as e:
            logger.warning(f"CLIP embedding failed, falling back to sparse retrieval only: {e}")
            qvec = None
            has_dense = False

        dense_hits, sparse_hits = [], []
        for shard in self._shards():
            if has_dense and qvec:
                try:
                    dense_hits.extend(self.store.search(self.modality, shard, qvec, top_k * 2))
                except Exception as e:
                    logger.warning(f"Vector search failed on shard {shard}: {e}")
            try:
                sparse_hits.extend(self.hybrid.sparse_search(self.modality, shard, query, top_k * 2))
            except Exception as e:
                logger.warning(f"Sparse search failed on shard {shard}: {e}")
        ...
        return self.reranker.rerank(query, fused, top_k=top_k, use_llm=use_llm_rerank)
...
agent = ImageAgent(rings, store, hybrid, reranker)
...
@app.get("/health")
def health():
    return {"status": "ok", "modality": "image", "shards": len(agent._shards()), "shard_nodes": agent._shards()}
