"""Table Agent Service — handles table modality retrieval."""
import logging
import os
...
app = FastAPI(title="AgentMesh Table Agent Service")
...
for i in range(settings.SHARD_NODES):
    rings.add_node("table", f"shard-{i}")
...
class TableAgent:
    ...
    def run(self, query, top_k=5, use_llm_rerank=False, embed_query_text=None):
        ...
        for h in hits:
            ...
            results.append(AgentResult(modality=self.modality, content=h["metadata"].get("content", ""), source=h["metadata"].get("source", "unknown"), raw_score=h.get("rerank_score", h["score"]), confidence=confidence))
        return results

agent = TableAgent(rings, store, hybrid, reranker)
...
@app.get("/health")
def health():
    return {"status": "ok", "modality": "table", "shards": len(agent._shards()), "shard_nodes": agent._shards()}
